    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_3654 = _Time;
                float2 node_7784 = ((float2(mul(unity_ObjectToWorld, v.vertex).r,mul(unity_ObjectToWorld, v.vertex).b)*_Scale)+node_3654.g*float2(0,0.1));
...
/////// Diffuse:
                ...
                float4 node_3654 = _Time;
                float2 node_7784 = ((float2(i.posWorld.r,i.posWorld.b)*_Scale)+node_3654.g*float2(0,0.1));
                float4 node_3153 = tex2D(_cloud,TRANSFORM_TEX(node_7784, _cloud));
                float3 diffuseColor = lerp(_Color.rgb,float3(node_3153.r,node_3153.r,node_3153.r),0.5);
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                VertexOutput o = (VertexOutput)0;
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 node_504 = _Time;
                float2 node_7784 = ((float2(mul(unity_ObjectToWorld, v.vertex).r,mul(unity_ObjectToWorld, v.vertex).b)*_Scale)+node_504.g*float2(0,0.1));
                float4 node_3153 = tex2Dlod(_cloud,float4(TRANSFORM_TEX(node_7784, _cloud),0.0,0));
                v.vertex.xyz += (node_3153.r*_strength*v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                float3 lightColor = _LightColor0.rgb;
...
/////// Diffuse:
                ...
                float3 directDiffuse = max( 0.0, NdotL) * attenColor;
                float4 node_504 = _Time;
                float2 node_7784 = ((float2(i.posWorld.r,i.posWorld.b)*_Scale)+node_504.g*float2(0,0.1));
                float4 node_3153 = tex2D(_cloud,TRANSFORM_TEX(node_7784, _cloud));
                float3 diffuseColor = lerp(_Color.rgb,float3(node_3153.r,node_3153.r,node_3153.r),0.5);
                float3 diffuse = directDiffuse * diffuseColor;
/// Final Color:
                float3 finalColor = diffuse;
                fixed4 finalRGBA = fixed4(finalColor * saturate(( saturate((sceneZ-partZ)/_Depth) > 0.5 ? (node_3153.r + 2.0*saturate((sceneZ-partZ)/_Depth) -1.0) : (node_3153.r + 2.0*(saturate((sceneZ-partZ)/_Depth)-0.5)))),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                VertexOutput o = (VertexOutput)0;
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 node_413 = _Time;
                float2 node_7784 = ((float2(mul(unity_ObjectToWorld, v.vertex).r,mul(unity_ObjectToWorld, v.vertex).b)*_Scale)+node_413.g*float2(0,0.1));
                float4 node_3153 = tex2Dlod(_cloud,float4(TRANSFORM_TEX(node_7784, _cloud),0.0,0));
                v.vertex.xyz += (node_3153.r*_strength*v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                o.pos = UnityObjectToClipPos( v.vertex );
